...
let web3 = new Web3(new Web3.providers.HttpProvider(NODE3))
let contractInst = new web3.eth.Contract(abi, contractAddress)  
contractInst.methods.name().call()
.then(su => console.log(`name (from node 3): ${su}`)); 

contractInst.methods.totalSupply().call()
.then(su => console.log(`Total supply (from node 3): ${su}`)); 

contractInst.methods.balanceOf(accountnode1).call()
.then(su => console.log(`balance Of (from node 3) account 1:${accountnode1}  ${su}`)); 

contractInst.methods.balanceOf(accountnode2).call()
.then(su => console.log(`balance Of (from node 3) account 2:${accountnode2}  ${su}`));

contractInst.methods.balanceOf(accountnode3).call()
.then(su => console.log(`balance Of (from node 3) account 3:${accountnode3}  ${su}`));

contractInst.methods.balanceOf(accountnode4).call()
.then(su => console.log(`balance Of (from node 3) account 4:${accountnode4}  ${su}`));
