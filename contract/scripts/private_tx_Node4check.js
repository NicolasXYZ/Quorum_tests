...
let web3 = new Web3(new Web3.providers.HttpProvider(NODE4))
let contractInst = new web3.eth.Contract(abi, contractAddress)  
contractInst.methods.name().call()
.then(su => console.log(`name (from node 4): ${su}`)); 

contractInst.methods.totalSupply().call()
.then(su => console.log(`Total supply (from node 4): ${su}`)); 

contractInst.methods.balanceOf(accountnode1).call()
.then(su => console.log(`balance Of (from node 4) account 1:${accountnode1}  ${su}`)); 

contractInst.methods.balanceOf(accountnode2).call()
.then(su => console.log(`balance Of (from node 4) account 2:${accountnode2}  ${su}`));

contractInst.methods.balanceOf(accountnode3).call()
.then(su => console.log(`balance Of (from node 4) account 3:${accountnode3}  ${su}`));

contractInst.methods.balanceOf(accountnode4).call()
.then(su => console.log(`balance Of (from node 4) account 4:${accountnode4}  ${su}`));

contractInst.methods.transfer(accountnode1, amount/8)
.send({
  from: accountnode4,
  privateFor: [nodePublicKey1, nodePublicKey2],
})
.then(res => {
  console.log(res)
})
