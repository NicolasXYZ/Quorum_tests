...
.then(su => console.log(`balance Of (from node 3):${accountnode1}  ${su}`)); 

contractInst.methods.balanceOf(accountnode2).call()
.then(su => console.log(`balance Of (from node 3):${accountnode2}  ${su}`));

contractInst.methods.balanceOf(accountnode3).call()
.then(su => console.log(`balance Of (from node 3):${accountnode3}  ${su}`));


contractInst.methods.transfer(accountnode1, amount/100)
  .send({
    from: accountnode3,
    privateFor: [nodePublicKey1, nodePublicKey3],
  })
  .then(res => {
    console.log(res)
  })
