...
.then(su => console.log(`name (from node 1): ${su}`)); 

contractInst.methods.totalSupply().call()
.then(su => console.log(`Total supply (from node 1): ${su}`)); 

contractInst.methods.balanceOf(accountnode1).call()
.then(su => console.log(`balance Of (from node 1) account 1:${accountnode1}  ${su}`)); 

contractInst.methods.balanceOf(accountnode2).call()
.then(su => console.log(`balance Of (from node 1) account 2:${accountnode2}  ${su}`));

contractInst.methods.balanceOf(accountnode3).call()
.then(su => console.log(`balance Of (from node 1) account 3:${accountnode3}  ${su}`));

contractInst.methods.transfer(accountnode2, amount/2)
  .send({
    from: accountnode1,
  ...
  contractInst.methods.balanceOf(accountnode3).call()
  .then(su => console.log(`balance Of (from node 1) accoun 3:${accountnode3}  ${su}`));
